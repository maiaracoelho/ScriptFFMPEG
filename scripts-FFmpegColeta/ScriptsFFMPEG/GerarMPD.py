...
for granul in granularities:
    ...
    for fps in fpss:
            ...
            files = sorted(out_file)
            for i in range(0,len(files)):
                    file = files[i].split("_")
                    #print file
                    if file[0]=="sintelV":
                        video_bitrate = file[1]
                        band = int(video_bitrate)*1000
                        files[i] = files[i]+"#video:bandwidth="+str(band)
                        print band
                    elif file[0]=="sintelA":
                        audio_bitrate = file[1]
                        band = int(audio_bitrate)*1000
                        files[i] = files[i]+"#audio:bandwidth="+str(band)
                        print band
